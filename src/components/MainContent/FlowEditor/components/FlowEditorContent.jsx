...
import ReactFlow, { MiniMap, Controls, Background, ConnectionLineType, Panel, useReactFlow } from "reactflow";
...
function FlowEditorContent({ initialMode, loadedData, filePath, fileName, tabId, onCreateNewTab, onUpdateTab, onRequestTabClose, onHistoryChange }) {
  ...
  // タブが表示されたときにReactFlowを初期化
  React.useEffect(() => {
    // 少し遅延させてReactFlowが完全にレンダリングされてから実行
    const timer = setTimeout(() => {
      try {
        setZoom(getZoom());
        // ノードがない場合でも、デフォルトのビューポートを設定
        if (nodes.length === 0) {
          // 空の状態でもズーム操作を可能にするため、中央にビューを設定
          setViewport({ x: 0, y: 0, zoom: 1 });
        } else {
          // ノードがある場合は通常通りfitView
          fitView({ duration: 200 });
        }
      } catch (error) {
        // ReactFlowがまだ完全に初期化されていない場合のエラーを無視
        console.debug("ReactFlow initialization pending:", error);
      }
    }, 100);

    return () => clearTimeout(timer);
  }, [tabId, getZoom, fitView, setViewport, nodes.length]); // setViewportを依存配列に追加
    ...
    <div className="h-full flex flex-col bg-gray-100">
      ...
      <div className="flex-1 relative w-full h-full" ref={reactFlowWrapper} onDrop={onDrop} onDragOver={onDragOver} onDragLeave={onDragLeave} onMouseMove={copyPaste.updateMousePosition}>
        <ReactFlow
          ...
          fitView={nodes.length > 0} // ノードがある場合のみfitViewを実行
